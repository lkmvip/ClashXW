...
namespace PortableModeUtil
{
	...
	void AskForPortableMode()
	{
		...
		int button;
		if (SUCCEEDED(TaskDialog(nullptr, g_hInst, _(L"Portable Mode"), nullptr, _(L"Do you want to enable portable mode?\nEnabling portable mode will disable non-portable features like URL protocol."), TDCBF_YES_BUTTON | TDCBF_NO_BUTTON, MAKEINTRESOURCEW(IDI_CLASHXW), &button)) && button == IDYES)
		{
			g_portableMode = true;
